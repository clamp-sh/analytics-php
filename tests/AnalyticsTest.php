...
final class AnalyticsTest extends TestCase
{
    ...
    public function testPropertyValueTypesRoundtrip(): void
    {
        ...
        // assertEquals (loose) instead of assertSame because PHP's
        // json_encode(29.0) emits "29" (no decimal), so the roundtripped
        // value is int 29, not float 29.0. The wire shape is correct;
        // strict identity is fragile here.
        $this->assertEquals(['amount' => 29.00, 'currency' => 'USD'], $props['total']);
    }
    ...
    public function testCaptureErrorSendsErrorEvent(): void
    {
        $this->captureTransport();
        Analytics::init('proj_test', 'sk_proj_test');

        try {
            throw new \RuntimeException('checkout failed: invalid plan');
        } catch (\RuntimeException $e) {
            Analytics::captureError($e, ['plan' => 'ultra', 'retry' => 1]);
        }

        $payload = json_decode($this->captured[0]['body'], true, 512, JSON_THROW_ON_ERROR);
        $this->assertSame('$error', $payload['name']);
        $props = $payload['properties'];
        $this->assertSame('checkout failed: invalid plan', $props['error.message']);
        $this->assertSame('RuntimeException', $props['error.type']);
        $this->assertStringContainsString('AnalyticsTest', $props['error.stack']);
        $this->assertTrue($props['error.handled']);
        $this->assertSame('ultra', $props['plan']);
        $this->assertSame(1, $props['retry']);
    }

    public function testCaptureErrorIgnoresHandledContextKey(): void
    {
        $this->captureTransport();
        Analytics::init('proj_test', 'sk_proj_test');
        Analytics::captureError(new \LogicException('oops'), ['handled' => false, 'ok' => true]);

        $payload = json_decode($this->captured[0]['body'], true, 512, JSON_THROW_ON_ERROR);
        $this->assertTrue($payload['properties']['error.handled']);
        $this->assertTrue($payload['properties']['ok']);
    }
